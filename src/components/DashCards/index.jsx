...
const DashCards = () => {
	...
	const percentualContracts = (
		((finishedContracts - finishedContractLastMonth) / (finishedContracts || 1)) *
		100
	).toFixed(2);
	//contratos pendentes
		...
		? (
				((monthIncoming -
					userContracts
						...
						.reduce(
							(acc, item) =>
								Number(item.service.finalValue) + acc,
							0
						)) /
					(monthIncoming || 1)) *
				100
	...
	const pendentContractsLastMonth = userContracts.length
		...
		: 0;
	const pendentsDiff = (
		((pendentsContracts - pendentContractsLastMonth) / (pendentsContracts || 1)) *
		100
	).toFixed(2);
    //taxa conversão (não é bem isso)
    const totalConvertions = userContracts.length ? userContracts.filter(
		(item) => new Date(item.startDate).getMonth() === new Date().getMonth()
	).length : 0;
    const clientsThisMonth = userClients.length
		? userClients.filter(
				(item) =>
					new Date(item.clientSince).getMonth() ===
					new Date().getMonth()
		  ).length
		: 0;
    const convertionsDiff = (totalConvertions / clientsThisMonth * 100).toFixed(2)
    ...
    const convertionCompare =
		...
		((totalConvertions / (clientsThisMonth || 1)) || 1)) * 100).toFixed(2);
	return (
			...
			<Card className={classes.root}>
				<CardContent className={classes.containerA}>
					...
					<CardContent className={classes.containerB}>
						...
						<Typography
							className={
								pendentsDiff ? classes.rateDown : classes.rate
							}
							color="textSecondary"
							gutterBottom
						>
							{pendentsDiff === "0.00" ? null : pendentsDiff >
							  0 ? (
								<TrendingUpIcon className={classes.arrowDown} />
							) : (
								<TrendingDownIcon className={classes.arrow} />
							)}{" "}
							{`${pendentsDiff}%`}
						</Typography>
					</CardContent>
				...
				<CardContent className={classes.containerA}>
					...
					<CardContent className={classes.containerB}>
						...
						>
							...
							"0.00" ? null : lastMonthIncomingDiff > 0 ? (
								<TrendingUpIcon className={classes.arrow} />
							) : (
								<TrendingDownIcon
									className={classes.arrowDown}
