  ...
  const getProfitsByServices = () => {
    ...
    const servicesNames = getAllServices("Names");
      ...
      for (let i = 0; i < servicesNames.length; i++){
          servicesIds.push(userServices.find(item => item.title === servicesNames[i]).id)
      }
    
      let values = servicesIds.map(item => userContracts.filter(serv=> serv.service.id === item).reduce((acc,contr)=> Number(contr.service.finalValue) + acc, 0))
